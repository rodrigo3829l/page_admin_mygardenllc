...
export const numberValidate = value =>{
    ...
    if (!/^\d+$/.test(value)) {
        return 'El valor debe contener solo dígitos numéricos';
    }
    ...
    // Si pasa ambas validaciones, retorna null para indicar que el valor es válido
    return '';
}
...
export const departmentValidate = value =>{
    return (value)
        ? ''
...
export const passwordValidate = value =>{
    if (value < 6) {
        return 'El password debe de ser mayo de 6 digitos'
    }

    // Check for at least one uppercase letter
    if (!/[A-Z]/.test(value)) {
        return 'El password debe de contener al menos una mayuscula'
    }

    // Check for at least one number
    if (!/\d/.test(value)) {
        return 'El password debe de tener almenos un caracter especial'
    }
    
    // Check for only one special character
    const specialCharCount = (value.match(/[^A-Za-z0-9]/g) || []).length;
    if (specialCharCount !== 1) {
        return 'El password debe de contener almenos un caracter especial'
    }
    return ''
}
