...
<v-container>
        ...
        <v-card-text>
            <v-data-table :headers="headers" :items="processedProjects">
                <template v-slot:item.actions="{ item }">
                    ...
                    <v-btn variant="text" @click="openDeleteDialog(item)" color="red-darken-3" prepend-icon="mdi-delete">
                        Eliminar
                    </v-btn>
                </template>
            </v-data-table>
    ...
    <v-dialog v-model="confirmDialog" width="500">
        <v-card>
            ...
            <v-card-text>¿Estás seguro de que quieres eliminar este proyecto?</v-card-text>
            <v-card-actions class="justify-end">
                <v-btn text @click="confirmDialog = false">Cancelar</v-btn>
                <v-btn color="red" @click="confirmDeleteProject">Eliminar</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</v-container>
    ...
    methods: {
        ...
        async confirmDeleteProject() {
            ...
            try {
                const {
                    data
                } = await api.delete(`/feature/delete/${this.projectToDelete._id}`, {
                    headers: {
                        Authorization: "Bearer " + token,
                        rol: "admin",
                    },
